...
public class MigPaneTest13 extends Application {
    ...
    public void start (Stage stage) throws Exception {
        ...
        final LabelBuilder<?> lBuilder = LabelBuilder
                .create()
                .text("Test long label to see if the wrap works ok in a Migpane. I am"
                        + " going to have to keep writing because this may not be long enough yet!!")
                .wrapText(true);

        final VBox root = new VBox();
        // we give a fixed width of 400 and a height of -1. The latter one means that the height of the Scene should be
        // calculated by the bounds of its root 
        final Scene scene = new Scene(root, 400, -1);

        final VBox vBox = new VBox();
        LabelBuilder<?> titleLB = LabelBuilder.create().style(
                "-fx-font-weight: bold;");
        ...
        mPane.add(titleLB.text("MigPane").build(), "wrap, hmin pref");
        ...
        // The label correctly calculetes its pref and max height based on the number of lines.
        // Eg setting wrapText to false returns its minWidth (height for 1 line) as prefWidth and cuts the rest of the
        // line with ellipsis, setting it to true returns a prefHeight so that every line is visible
